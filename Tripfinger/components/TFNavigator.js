...
var TFRCTNavigatorManager = require('NativeModules').TFRCTNavigatorManager;
 ...
 *
 */
var TFNavigator = React.createClass({
  ...
  componentWillReceiveProps: function(nextProps:any) {
    if (nextProps.navigationBarHidden !== this.props.navigationBarHidden) {
      if (this.refs[NAVIGATOR_ITEM_REF]) {
         this.refs[NAVIGATOR_ITEM_REF].setNativeProps({navigationBarHidden: nextProps.navigationBarHidden});
       }
     }
 },
  ...
  _rightButtonPressed(event) {
    if (this._component.rightButtonPressed) {
      this._component.rightButtonPressed(event.nativeEvent.action);
    }
  },
...
var TFRCTNavigator = requireNativeComponent('TFRCTNavigator');
var TFRCTNavigatorItem = requireNativeComponent('TFRCTNavItem');

module.exports = TFNavigator;
