...
NSInteger tfkNeverRequested = -1;
NSInteger tfkNeverProgressed = -10000;
...
@interface TFRCTNavigationController : UINavigationController <UINavigationBarDelegate>
{
 ...
 *
 */
@implementation TFRCTNavigationController
...
@interface TFRCTNavigator() <TFRCTWrapperViewControllerNavigationListener, UINavigationControllerDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, copy, readwrite) NSArray<TFRCTNavItem *> *previousViews;
@property (nonatomic, readwrite, strong) TFRCTNavigationController *navigationController;
/**
...
- (instancetype)initWithBridge:(RCTBridge *)bridge
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _bridge = bridge;
    _mostRecentProgress = tfkNeverProgressed;
    _dummyView = [[UIView alloc] initWithFrame:CGRectZero];
    _previousRequestedTopOfStack = tfkNeverRequested; // So that we initialize with a push.
    _previousViews = @[];
    __weak TFRCTNavigator *weakSelf = self;
    _navigationController = [[TFRCTNavigationController alloc] initWithScrollCallback:^{
                    ...
                    animated:(__unused BOOL)animated
{
  ...
  [tc animateAlongsideTransition: ^(id<UIViewControllerTransitionCoordinatorContext> context) {
    TFRCTWrapperViewController *fromController =
    (TFRCTWrapperViewController *)[context viewControllerForKey:UITransitionContextFromViewControllerKey];
    TFRCTWrapperViewController *toController =
    (TFRCTWrapperViewController *)[context viewControllerForKey:UITransitionContextToViewControllerKey];
 ...
 */
- (void)insertReactSubview:(TFRCTNavItem *)view atIndex:(NSInteger)atIndex
{
  RCTAssert([view isKindOfClass:[TFRCTNavItem class]], @"TFRCTNavigator only accepts TFRCTNavItem subviews");
  RCTAssert(
            _navigationController.navigationLock == RCTNavigationLockJavaScript,
...
- (void)removeReactSubview:(TFRCTNavItem *)subview
{
...
- (void)reactBridgeDidFinishTransaction
{
  ...
  _previousRequestedTopOfStack == tfkNeverRequested ? 0 : _previousRequestedTopOfStack + 1;
  ...
  if (jsGettingAhead) {
    if (reactPushOne) {
      ...
      TFRCTWrapperViewController *vc = [[TFRCTWrapperViewController alloc] initWithNavItem:(TFRCTNavItem *)lastView];
