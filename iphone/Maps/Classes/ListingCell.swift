...
class ListingCell: UITableViewCell {
  ...
  func heartClick() {
    if let notes = listing.listing.notes where notes.likedState == GuideListingNotes.LikedState.LIKED {
      DatabaseService.saveListingLike(GuideListingNotes.LikedState.SWIPED_LEFT, listing: listing)
    } else {
      DatabaseService.saveListingLike(GuideListingNotes.LikedState.LIKED, listing: listing)
    }
    setContent(listing)
  }
}